...
1. 首先在桌面创建一个 `office` 文件夹
2. 打开 `部署工具` 选择 `office` 文件夹将会下载两个文件在此文件中，需要删除 `configuration-Office365-x64` 文件;

## 步骤二 [设置`office`版本](https://config.office.com/deploymentsettings)
...
1. 管理员运行 `cmd`, 并将桌面上的 `office` 路径复制到终端按回车

```bash
# 根据自己的实际路径修改
cd C:\Users\你的用户名\Desktop\office
